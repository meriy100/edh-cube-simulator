...
export async function GET() {
  try {
    const pools = await prisma.pool.findMany({
      ...
      select: { id: true, title: true, createdAt: true, updatedAt: true, _count: { select: { poolCards: true } } },
    })
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < need.length; i += CHUNK_SIZE) {
      ...
      if (updates.length) {
        await prisma.$transaction(
          updates.map((u) => prisma.card.update({ where: { name: u.name }, data: { scryfallJson: u.json } }))
        )
    ...
    const cards = await prisma.card.findMany({ where: { name: { in: names } }, select: { id: true, name: true } })
