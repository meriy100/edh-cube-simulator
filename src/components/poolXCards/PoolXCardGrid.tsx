  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4">
      {poolXCards.map((poolXCard) => (
        <div
          key={poolXCard.name}
          className="flex flex-col space-y-2 p-2  bg-white dark:bg-gray-800 rounded-lg shadow-sm border dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow"
        >
          <CardImage card={poolXCard.card} href={`/admin/cards/${newCardId(poolXCard.name)}`} />
